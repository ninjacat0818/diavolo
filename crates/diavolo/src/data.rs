...
use super::visiting_state::{
    CallVisitingState, GotoVisitingState, ReturnVisitingState, VisitingState,
};
...
impl Data {
    ...
    pub(crate) fn goto(&mut self, nodes: &Nodes) {
        let line_id_or_index = self
            .visiting_state_unchecked::<GotoVisitingState>()
            .line_id_or_index
            .as_str();
    ...
    pub(crate) fn call(&mut self, nodes: &Nodes) {
        let node_key = self
            ...
            .unwrap_or_else(|| {
                self.visiting_state_unchecked::<CallVisitingState>()
                    .node_key
                    .clone()
            });
        let node = nodes
    ...
    pub(crate) fn ret(&mut self) {
        let value = self
            .visiting_state::<ReturnVisitingState>()
            .map(|r| &r.value)
            .cloned()
            .unwrap_or_default();
        self.state_machine.call_stack.pop();
        if !self.state_machine.call_stack.is_empty() {
            self.visiting_state_mut_unchecked::<CallVisitingState>()
                .ret(value);
        }
    }
    ...
    pub(crate) fn lines_unchecked(&self) -> &Lines {
        self.visiting_states
            .lines(self.state_machine.current_node_key())
            .expect("Lines should exist for the current node key")
    }

    pub(crate) fn complete_message_unchecked(&mut self, view: &View) {
        use super::visiting_state::{ChoiceVisitingState, MessageVisitingState};
        match view {
            View::Message(_) => {
                self.visiting_state_mut_unchecked::<MessageVisitingState>()
                    .complete();
            }
            View::Choice(_) => {
                self.visiting_state_mut_unchecked::<ChoiceVisitingState>()
                    .complete_message_unchecked();
            }
            _ => panic!("No message to complete in the current view"),
        }
    }

    pub(crate) fn visiting_state<T: VisitingState>(&self) -> Option<&T> {
        self.visiting_states
            .visiting_state::<T>(self.state_machine.location())
    }

    pub(crate) fn visiting_state_unchecked<T: VisitingState>(&self) -> &T {
        self.visiting_state::<T>().unwrap()
    }

    pub(crate) fn visiting_state_mut<T: VisitingState>(&mut self) -> Option<&mut T> {
        self.visiting_states
            .visiting_state_mut::<T>(self.state_machine.location())
    }

    pub(crate) fn visiting_state_mut_unchecked<T: VisitingState>(&mut self) -> &mut T {
