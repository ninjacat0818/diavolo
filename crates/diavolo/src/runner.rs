...
use dialogue::{ChoiceKey, Dialogue, LineType};
...
impl<'engine, 'dialogue> Runner<'engine, 'dialogue> {
    ...
    pub fn update_view(&mut self) -> Option<&View<'static>> {
        let data = &mut self.store.data.lock().unwrap();

        let updated = self
            .view
            .update(&self.store.engine, data, &self.dialogue.nodes);
        ...
        if updated && self.view.has_message_finished() {
            ...
            data.complete_message_unchecked(&self.view);
            self.view
                .update(&self.store.engine, data, &self.dialogue.nodes);
        }
...
impl Runner<'_, '_> {
    ...
    fn advance(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        loop {
            ...
            match self.evaluate()? {
                NextAdvance::Stop => break Ok(()),
                NextAdvance::Advance => (),
                NextAdvance::SkipAdvance => skipped = Some(()),
            }
        }
    }

    fn evaluate(&mut self) -> Result<NextAdvance, Box<dyn std::error::Error>> {
        ...
        loop {
            ...
            if !line_if_result {
                ...
                break Ok(NextAdvance::SkipAdvance);
            }
            ...
            let evaluated_line = {
                match line_type {
                    ...
                    LineType::Exit(exit) => {
                        ...
                        break Ok(NextAdvance::Advance);
                    }
                    _ => unreachable!("LineType not supported for evaluation"),
                }
            };
