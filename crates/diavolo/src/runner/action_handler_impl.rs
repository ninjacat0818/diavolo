...
use super::super::visiting_state::{ChoiceVisitingState, MessageVisitingState};
use super::Runner;
...
impl Runner<'_, '_> {
    ...
    pub(super) fn handle_skip(self: &mut Self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match view {
            View::Message(_) => {
                data.visiting_state_mut_unchecked::<MessageVisitingState>()
                    .skip();
                Ok(())
            }
            View::Choice(_) => data
                .visiting_state_mut_unchecked::<ChoiceVisitingState>()
                .try_skip_message(),
    ...
    pub(super) fn handle_select(
        ...
        match view {
            View::Choice(choice) => {
                ...
                data.visiting_state_mut_unchecked::<ChoiceVisitingState>()
                    .select(choice_key);
                drop(data);
