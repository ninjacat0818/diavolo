use super::data::Data;
use dialogue::{ChoiceTexts, LangTexts, LineIf, Text};
...
impl BoaCtx {
    ...
    pub fn eval_texts(&mut self, texts: &LangTexts) -> JsResult<LangTexts> {
        texts
            .iter()
            .map(|(language, text)| Ok((language.clone(), self.eval_text(text)?)))
            .collect()
    }
    ...
    fn eval_text(&mut self, text: &Text) -> JsResult<Text> {
        let result = self
            .context
    ...
    pub fn define_properties(&mut self, data: Arc<Mutex<Data>>) -> JsResult<bool> {
        let getter = unsafe {
            NativeFunction::from_closure(
                move |_this: &JsValue,
                    ...
                    for (index, (line_id, visiting_counting)) in lines.iter().enumerate() {
                        ...
                        if let Some(selected) = visiting_counting.selected() {
                            ...
                            let value = {
                                ...
                                let timestamp_ms = SystemTime::now()
                                    ...
                                    .sub(selected.selected_at.elapsed())
                                    .as_millis();
                                let date_ctor = context
                                    ...
                                    .unwrap();
                                date_ctor.construct(
                                    &[JsValue::from(timestamp_ms as f64)],
                                    None,
                                    context,
                                )?
                            };
        ...
        let descriptor = PropertyDescriptor::builder()
            .get(getter.to_js_function(self.context.realm()))
            .enumerable(true)
            .configurable(false)
            .build();

        self.context.global_object().define_property_or_throw(
            js_string!("lines"),
            descriptor,
            &mut self.context,
        )
