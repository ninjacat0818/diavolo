...
impl FromIterator<(ChoiceKey, LangTexts)> for ChoiceTexts {
    fn from_iter<T: IntoIterator<Item = (ChoiceKey, LangTexts)>>(iter: T) -> Self {
...
impl<'de> Deserialize<'de> for ChoiceTexts {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for Visitor {
            ...
            {
                let inner = Vec::<LangTexts>::deserialize(SeqAccessDeserializer::new(seq))?
                    .into_iter()
                    .enumerate()
                    .map(|(i, lang_texts)| (ChoiceKey(i.to_string()), lang_texts))
                    .collect::<IndexMap<ChoiceKey, LangTexts>>();
                if inner.is_empty() {
            ...
            {
                let inner: IndexMap<ChoiceKey, LangTexts> =
