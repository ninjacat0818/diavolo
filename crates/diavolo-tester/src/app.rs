    ...
    'engine: 'dialogue,
{
    ...
    fn update(&mut self, action: Action) -> Option<Action> {
        ...
        match action {
            ...
            Action::CheckView => {
                if self.runner.update_view() {
                    let view = self.runner.view();
                    tracing::trace!("View state changed: {:?}", view);
                    if view.has_message_finished() {
                        return Some(Action::CompleteMessage);
                    }
                    self.choice_list = view.choices().map(ChoiceList::from_iter);
                }
            }
            Action::FastForward => self.runner.dispatch(RunnerAction::ToggleFastForward),
            Action::SkipMessage => self.runner.dispatch(RunnerAction::Skip),
            Action::CompleteMessage => self.runner.dispatch(RunnerAction::CompleteMessage),
            Action::NextLine => self.runner.dispatch(RunnerAction::NextLine),
            ...
            Action::ConfirmChoice => {
                let key = self
                    ...
                    .expect("choice_list must be present when selecting choice");
